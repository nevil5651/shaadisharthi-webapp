...
/**
 * Servlet implementation for email verification initiation
 * Handles email verification token generation and email sending for customer registration
 * 
 * @WebServlet Maps to "/cstmr-verify-email" endpoint
 * @version 1.0
 * @description Processes POST requests to send verification emails with JWT tokens
 */
@WebServlet("/cstmr-verify-email")
public class EmailVerificationServlet extends HttpServlet {
    ...
    /**
     * Checks if email is rate limited based on recent attempts
     * 
    ...
    /**
     * Logs verification attempt for rate limiting
     * 
     * @param con Database connection
    ...
    /**
     * Processes POST requests for email verification initiation
     * Generates JWT tokens, stores them, and sends verification emails
     * 
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        logger.debug("Processing POST request for /verify-email");
        ...
        if (email == null || email.trim().isEmpty() || !email.matches("^[\\w-\\.]+@([\\w-]+\\.)+[\\w-]{2,4}$")) {
            ...
            responseJson.put("error", "Invalid email address");
            sendResponse(response, HttpServletResponse.SC_BAD_REQUEST, responseJson);
            return;
        }
        ...
        try (Connection con = DbConnection.getCon()) {
            ...
            try {
                ...
                try (PreparedStatement pstmt = con.prepareStatement(checkQuery)) {
                    ...
                    try (ResultSet rs = pstmt.executeQuery()) {
                        if (rs.next()) {
                            ...
                            con.rollback(); // No need to log attempt if duplicate
                            return;
                        }
                    }
                }
                ...
                // Store verification token in database with 15-minute expiry
                String insertQuery = "INSERT INTO email_verification_tokens (email, token, expiry, status) VALUES (?, ?, DATE_ADD(NOW(), INTERVAL 15 MINUTE), 'pending')";
                try (PreparedStatement pstmt = con.prepareStatement(insertQuery)) {
                    ...
                    logger.error("Error in storing token {}", e);
                }
                ...
                try {
                    ...
                    con.rollback();
                    responseJson.put("error", "Failed to send verification email");
                    sendResponse(response, HttpServletResponse.SC_INTERNAL_SERVER_ERROR, responseJson);
                    return;
                }

                con.commit();
                responseJson.put("message", "Verification email sent");
